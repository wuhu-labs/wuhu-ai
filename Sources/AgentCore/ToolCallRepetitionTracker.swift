...
/// After a tool call completes, call
/// ``record(toolName:argsHash:resultHash:)`` to update the streak for
/// that slot. If the result hash matches the previous result for the
/// same slot, the counter increments; otherwise it resets to 1.
///
/// Before executing a tool call, query
/// ``preflightCount(toolName:argsHash:)`` to decide whether to warn
/// or block.
///
/// - At count ≥ ``warningThreshold`` (3): callers should append a
///   warning.
...
public struct ToolCallRepetitionTracker: Sendable {
  ...
  ///
  /// Returns 0 if the pair has never been seen. Use this before
  /// execution to decide whether to block
  /// (count ≥ ``blockThreshold``).
