...
struct IntegrationTestRecordingContext {
  ...
  private func makeRecordedRequest(from request: Request) async throws -> RecordedRequest {
    ...
    return RecordedRequest(
      ...
      headers: RecordedHeader.capture(request.headers),
      body: body
    )
  }
...
private struct RecordedResponse: Codable, Equatable, Sendable {
  ...
  init(response: Response) async throws {
    ...
    self.body = try await RecordedBody.capture(response.body)
      ?? .utf8("")
  }
...
private enum RecordingError: Error, CustomStringConvertible {
  ...
  var description: String {
    ...
    case let .missingRecording(path):
      return "Missing integration recording at \(path). Delete the request file only when you want to re-record, and set \(recordingModeEnvironmentKey)=1 to record missing snapshots."
    ...
    case .recordingModeUnsupported:
      return "Recording mode requires FetchURLSession, which is only enabled for local macOS recording in this test harness."
    }
  }
}
